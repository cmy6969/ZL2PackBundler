...
- Icon replacement: `--icon <png|jpg|webp>` resizes the image to each density bucket's original dimensions and replaces the mipmap bitmaps (webp/png), then removes the anydpi adaptive-icon XMLs (on Android 8+ the launcher falls back to the replaced bitmaps; the trade-off is losing system "themed icon" support). APKs with vector-only icons (no bitmap buckets) cannot be replaced and are reported as skipped.
